...
def extract_employees_date_births(df):
    try:
        # Strip leading/trailing whitespace from date strings
...
def calculate_employees_age(date_of_birth):
    ...
    for dob in date_of_birth:
        try:
            # Convert to datetime, with dayfirst=True for day/month/year format
    ...
    if df is not None:
        ...
        if date_of_birth is not None:
            ...
            if ages:
                ...
                valid_indices = [i for i, y in enumerate(years_until_retirement_list) if y is not None]
                filtered_ages = [ages[i] for i in valid_indices]
                filtered_years_until_retirement = [years_until_retirement_list[i] for i in valid_indices]
                
                for age, years_left in zip(filtered_ages, filtered_years_until_retirement):
                    print(f"Age: {age}, Years until retirement: {years_left}")
                
                # Plot 1: Age Distribution
                plt.figure(figsize=(10, 6))
                plt.hist(ages, bins=range(20, 80, 5), color="skyblue", edgecolor="black")
                ...
                plt.figure(figsize=(10, 6))
                plt.bar(filtered_ages, filtered_years_until_retirement, color="orange", edgecolor="black")
                ...
                retirement_years = [
                    current_year + y if y is not None else current_year for y in years_until_retirement_list
                ]
                retirement_years_sorted = sorted(retirement_years)
                cumulative_retirement = range(1, len(retirement_years_sorted) + 1)

                plt.figure(figsize=(10, 6))
                plt.plot(retirement_years_sorted, cumulative_retirement, marker="o", color="purple")
